...
# 3. Workload Identity Federation 設定（GitHub Actions → GCP の認証）
...
> **Tip:** Step 6 の `git push` で GitHub Actions が自動的に Docker build → Artifact Registry push → Cloud Run deploy を実行します。これが AI Studio の「Publish」ボタンに相当します。

#### 2回目以降のデプロイ

```bash
# コードを変更して push するだけ — CI/CD が自動デプロイ
git add .
git commit -m "Update feature X"
git push origin main
```

GitHub Actions (`.github/workflows/deploy.yml`) が以下を自動実行します:
